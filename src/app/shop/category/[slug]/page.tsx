...
export default async function CategoryPage({
  ...
  // Category Ke Products Fetch Karne Ke Liye Query
  ...
  return (
    <div className="p-4 m-10">
      <h1 className="text-2xl text-[#FF9F0D] font-bold mb-10">Category : {slug}</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {products.map((product) => (
          <div key={product.slug.current} className="border p-4 rounded-lg shadow-md">
...
// Static Paths Generate Karne Ke Liye
export async function generateStaticParams(): Promise<{ slug: string }[]> {
  ...
  // Debugging: Log the generated static paths
  console.log("Generated Static Paths:", categories.map((category) => ({
    slug: category.slug.current,
  })));

  return categories.map((category) => ({
    slug: category.slug.current, // Slug ka current value use karein
  }));
}